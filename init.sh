...
if [ ! -f ./.ssh/id_rsa ]; then
   su gitolite -c "ssh-keygen -f /home/gitolite/.ssh/id_rsa  -t rsa -N ''"
...
if [ ! -f ./.ssh/known_hosts ]; then
    if [ -n "$TRUST_HOSTS" ]; then
        ...
        su gitolite -c "ssh-keyscan -H $TRUST_HOSTS > /home/gitolite/.ssh/known_hosts"
    fi
fi
...
if [ ! -d ./.gitolite/conf ] ; then
   ...
   if [ -n "$SSH_KEY" ]; then
       ...
       su gitolite -c "bin/gitolite setup -pk /tmp/admin.pub"
       rm /tmp/admin.pub
   else
       ...
       su gitolite -c "bin/gitolite setup -a dummy"
       ...
       su gitolite -c "cd /home/gitolite/repositories/gitolite-admin.git && GL_LIBDIR=$(/home/gitolite/bin/gitolite query-rc GL_LIBDIR) PATH=$PATH:/home/gitolite/bin hooks/post-update refs/heads/master"
   fi
else
